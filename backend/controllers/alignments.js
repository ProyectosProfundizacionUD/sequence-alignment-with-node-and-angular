...
const ValidateHeaders = (headers) => {
  if (headers["x1"] && headers["x2"] && headers["y1"] && headers["y2"])
    if ((Number.parseInt(headers["x1"])) <= (Number.parseInt(headers["x2"])) && 
...
const LocalAlignment = async (headers, sequence, organism, res) => {
  ...
  sequence = sequence.substring(parseInt(headers["x1"]), parseInt(headers["x2"]) + 1);
  sequences.forEach(seq => {
    seq.sequence = seq.sequence.substring(parseInt(headers["y1"]), parseInt(headers["y2"]) + 1); 
  });

  if( ( Number.parseInt(headers["x2"]) - Number.parseInt(headers["x1"]) ) <  
        Number.parseInt(headers["y2"]) - Number.parseInt(headers["y1"]) ) {
    let lengthToApply = (Number.parseInt(headers["y2"]) - Number.parseInt(headers["y1"]) - 
    ...
    let lengthToApply = (Number.parseInt(headers["x2"]) - Number.parseInt(headers["x1"]) - 
      (Number.parseInt(headers["y2"]) - Number.parseInt(headers["y1"])));
